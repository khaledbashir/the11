...
export async function POST(request: NextRequest) {
  try {
    const { messages, workspaceSlug = 'sow-master-dashboard', mode = 'chat' } = await request.json();
    ...
    console.log('🔵 [AnythingLLM Chat] Sending to workspace:', workspaceSlug);
    ...
    const response = await fetch(`${ANYTHINGLLM_URL}/api/v1/workspace/${workspaceSlug}/chat`, {
      method: 'POST',
      headers: {
